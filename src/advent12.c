...
void get12b(char *f) {
    ...
    int done[3] = { };
    Moon ds0[4];
    memcpy(ds0, moons, 4 * sizeof(Moon));
    ...
    while (1) {
        ...
        for (int x = 0; x < 3; x++)
            if (done[x] == 0) {
                int found_repeat = 0;
                for (int k = 0; k < 4; k++)
                    if (moons[k].v[x] == 0 && moons[k].x[x] == ds0[k].x[x])
                        found_repeat += 1;
    ...
    uint64_t result = (uint64_t)(done[0] / ggt(done[0], done[1])) * (done[1] / ggt(done[0], done[2]))
            * (done[2] / ggt(done[2], done[1])) * 2;
